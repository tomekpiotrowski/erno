...
- [ ] **[APP]** **Privacy data declarations** — Document what Erno collects by default (email, device tokens, stored files, sync data) and provide a pre-filled template/checklist for iOS Nutrition Labels and Android Data Safety; the actual declaration is submitted by the account holder per app
  - **Now also covers error reporting** (`errorReporting` in `provideErno`): when enabled it collects **Diagnostics → Crash Data** and **Identifiers → User ID**, linked to the user, and sends them to the monitoring collector — which in production is a *different provider*, making it a processor relationship for the privacy policy. Off unless an ingest key is configured. See `docs/src/content/docs/app/error-reporting.md` for exactly what is and is not collected.
